...
public class HashTable<E>
{
     ...
     */
    public boolean insert(E x, E followWord) {
        // Insert x as active
        int currentPos = findPos(x);
        if(isActive(currentPos)) {
            array[currentPos].wordFreq.updateFollows(followWord.toString());
            return false;
        }
    ...
    public String toString (int limit){
        ...
        for (int i=0; i < array.length && ct < limit; i++){
            if (array[i]!=null && array[i].isActive) {
                ...
                // prints out the following words
               // sb.append(array[i].wordFreq.toString() + "\n");
                ct++;
            }
        }
        return sb.toString();
    ...
    private void rehash( )
    {
        ...
        for( HashEntry<E> entry : oldArray )
            if( entry != null && entry.isActive ) {
                ...
                insert(entry.element, (E) "");
                array[findPos(entry.element)].wordFreq.followList = entry.wordFreq.followList;
            }
    }
    ...
    private int findPos( E x )
    {
        ...
        while(array[currentPos] != null &&
                !array[ currentPos ].element.equals(x) && array[currentPos].isActive)
        {
    ...
    public String randFollow(E x)
    {
        ...
        if (!isActive( currentPos )) {
            ...
            return array[currentPos].wordFreq.pickNextWord();
        }
    }
    ...
    private static class HashEntry<E> {
        ...
        public HashEntry( E e, boolean i ) {
            element  = e;
            isActive = i;
            wordFreq = new WordFreqInfo(this.element.toString(), 0);
        }
    }

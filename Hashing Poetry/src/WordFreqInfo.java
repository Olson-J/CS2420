...
public class WordFreqInfo {
    ...
    public String pickNextWord(){
        int index = (int) Math.floor(Math.random() * occurCt);
        int count = 0;
        while (count < followList.size()){
            if (this.followList.get(count).followCt > index){
                return this.followList.get(count).follow;
            }
            index = index - this.followList.get(count).followCt;
            count++;
        }
        return "uh oh";
